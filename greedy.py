...
def main():
    ...
    for d in tqdm(data):
        ...
        if args.word_order == 'seq':
            ...
            for i, j in indices:
                tmp = tokens[i][j]
                attack_texts = []
                synonyms = synonym_query(tokens[i][j], tags[i][j], word_case_recover=True)
                for syn in synonyms:
                    tokens[i][j] = syn
                    if isinstance(text, str):
                        cur_text = nltk_detokenize(tokens[i])
                ...
                num_model_queries += len(attack_texts)
                attack_prob = torch.softmax(torch.from_numpy(attack_pred), dim=-1)[:, label]
                attack_effect.append(orig_prob[label] - torch.min(attack_prob))
                tokens[i][j] = tmp
            attack_effect = torch.tensor(attack_effect)
            h = saliency * attack_effect
            idx = torch.argsort(h, descending=True).tolist()
            indices = [indices[i] for i in idx]
        else:
    ...
    with open(os.path.join(args.output_path, f'{args.word_order}.txt'), 'w', encoding='utf8') as f:
        print(f'asr {asr:.3f}, model query {num_model_queries:.0f}, '
              f'sem sim {sem_sim:.3f}, ppl {ppl:.1f}, '
